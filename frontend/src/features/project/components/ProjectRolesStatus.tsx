...
import { ProjectRoleStatus, type ProjectRoleStatusResponse } from '../project.types';
...
export const ProjectRolesStatus = ({ projectId }: { projectId: string }) => {
    ...
    const getStatusBadge = (status: ProjectRoleStatus) => {
        switch (status) {
            case ProjectRoleStatus.OPEN:
                return <Badge className="bg-green-100 text-green-800 border-green-200">Otwarte</Badge>;
            case ProjectRoleStatus.PENDING:
                return <Badge className="bg-yellow-100 text-yellow-800 border-yellow-200">Oczekujący wniosek</Badge>;
            case ProjectRoleStatus.FILLED:
                return <Badge className="bg-blue-100 text-blue-800 border-blue-200">Zajęte</Badge>;
            default:
                        ...
                        {roles?.map((role) => (
                            <div key={role.id} className="flex flex-col p-4 border rounded-lg shadow-sm bg-background hover:border-primary/50 transition-colors">
                                ...
                                <div className="mt-4 flex justify-end">
                                    {role.status === ProjectRoleStatus.OPEN && (
                                        <Button
                                            variant="outline"
                                            size="sm"
