...
export const ProjectHeader = ({details}: ProjectHeaderProps) => {
  ...
  const wallet = groups?.wallets.find(w => w.id === details.groupId);
  const program = groups?.programs.find(p => p.id === details.groupId);
  
  const groupName = wallet?.name || program?.name;
